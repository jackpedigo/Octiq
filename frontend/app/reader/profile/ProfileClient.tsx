    ...
    const loadProfile = async () => {
      ...
      try {
        ...
        setSelectedInterests(
          (data.interests || []).map((x: string) =>
            normalizeInterestForDisplay(x)
          )
        );
      } catch (err: any) {
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="block space-y-2">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-700">
                {computedEvidenceVisibility.charAt(0).toUpperCase() +
                  computedEvidenceVisibility.slice(1)}
              </div>
              <div className="text-xs text-slate-500">
                Auto-calculated from Depth and News Literacy.
              </div>
            </div>
          </div>
